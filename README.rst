...
``match``, ``preprocessor``, and ``postprocessor`` (explained later). However, if you
prefer writing a plugin as a class, you can subclass the included ``Plugin``, ``Command``,
or ``Match`` base classes, provided you have followed the above rules. Here is a simple example:
...
Similar to preprocessors, postprocessors can modify the responses generated by plugins prior to
those messages being sent over IRC. Postprocessors should accept arguments (in order) for
``client``, ``channel``, ``nick``, ``message``, and ``responses``. The ``responses`` argument
could be None, a string, or a list of strings. Postprocessors should return a single value that
is either None, a string, or a list of strings. To declare a function as a postprocessor, a
convenient decorator can be used.

.. code-block:: python

    from helga.plugins import postprocessor

    @postprocessor
    def blank_message(client, channel, nick, message, responses):
        return None
...
.. code-block:: python
    ...
    def complex(client, channel, nick, message, *args):
        ...
        # len(args) == 1 for matches and postprocessors
...
run first and postprocessors will always run last. A priority value should be an integer value.
There are no limits or bounds for this value, but know that a higher value will mean a higher priority.
If you are writing ``Plugin`` subclass style plugins, you will need to set a ``priority`` attribute
of your object. This is done automatically if you call ``super(MyClass, self).__init__(priority=some_value)``
...
.. code-block:: python
    ...
    from helga import command, match, preprocessor, postprocessor

    @preprocessor(priority=10)
    def foo_preprocess(*args):
        pass

    @postprocessor(priority=20)
    def foo_postprocess(*args):
        pass

    @command('foo', priority=30)
    def foo_command(*args):
        pass

    @match(r'foo', priority=40)
    def foo_match(*args):
        pass
...
- Postprocessors have default priority of ``PRIORITY_NORMAL``
...
| Plugin     | Description                                          |
+============+======================================================+
| `excuses`_ | Generate a response from http://developerexcuses.com |
+------------+------------------------------------------------------+
| `isup`_    | Check downforeveryoneorjustme.com                    |
+------------+------------------------------------------------------+
| `karma`_   | Dish out karma points to other people                |
+------------+------------------------------------------------------+
| `norris`_  | Generate Chuck Norris facts for users                |
+------------+------------------------------------------------------+
| `updates`_ | List and record IRC channel updates.                 |
+------------+------------------------------------------------------+
| `zen`_     | The Zen of Python                                    |
+------------+------------------------------------------------------+

.. _`excuses`: https://github.com/alfredodeza/helga-excuses
.. _`isup`: https://github.com/shaunduncan/helga-isup
.. _`karma`: https://github.com/coddingtonbear/helga-karma
.. _`norris`: https://github.com/alfredodeza/helga-norris
.. _`updates`: https://github.com/cobbdb/helga-contrib-updates
.. _`zen`: https://github.com/shaunduncan/helga-zen
